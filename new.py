...
response_extract = requests.post(url+f"?text={text}")
...
if response_extract.status_code == 200:
    ...
    text_to_check = {"text_to_check": description}
    url_quantification = "http://127.0.0.1:8000/quantification"
    
    # Send the data to the quantification endpoint
    response_quantification = requests.post(url_quantification, json=text_to_check)
